...
void displayStat() {
  pushToConsole(string_format("max used memory: %ld maxstack:%ld  started %d "
                              "free mem:%ld consumed %ld time:%dms",
                              __maxMemUsage, __MaxStackMemory, __startmem,
...
public:
  ...
  NodeToken *addChild(NodeToken j) {
    ...
    if (tmp == NULL) {
      // printf("impossinlme pour crear %s
  ...
  void clear(bool all) {
    ...
    int i = 0;
    if (!all) {
      if (_nodetype == (int)callFunctionNode ||
          _nodetype == (int)extCallFunctionNode ||
          _nodetype == (int)defAsmFunctionNode ||
          _nodetype == (int)defFunctionNode ||
          _nodetype == (int)defExtFunctionNode) {
        // printf("on tente %s\r\n",getTokenText());
        i = 2;
      }
    }
...
public:
  ...
  void addFunction(NodeToken *nd) { _functions.push_back(*nd); }
  void addVariable(NodeToken nd) { variables.push_back(nd); }
...
void _visitoperatorNode(NodeToken *nd) {
  ...
  case TokenPlusPlus:
    if (nd->getChildAtPos(0)->isPointer &&
        nd->getChildAtPos(0)->children.size() == 0) {
      bufferText->addAfter(string_format(
...
void _visitglobalVariableNode(NodeToken *nd) {
  ...
  if (nd->asPointer) //(&d)
  {
    ...
    bufferText->sp.push(bufferText->get());
  } else if ((nd->children.size() > 0 or !nd->isPointer) &&
             nd->type !=
    ...
    if (nd->type == TokenUserDefinedVariableMember or
      ...
      if (!nd->isPointer) {
        ...
        bufferText->addAfter(string_format(
            "%s %s%d,%s%d,%d", asmInstructionsName[asmInstr].c_str(),
            getRegType(asmInstr, 0).c_str(), register_numl.get(),
            getRegType(asmInstr, 1).c_str(), 3, start / 1000));
...
void _visitdefFunctionNode(NodeToken *nd) {
  ...
  if (!isStructFunction) {
    ...
    header.addAfter(string_format(
        ".var %d%s", nd->getChildAtPos(1)->children.size(), variables.c_str()));
  }

  header.addAfter(string_format("@_stack_%s:", nd->getTokenText()));
  header.addAfter(string_format(
      ".bytes %d", (nd->getChildAtPos(1)->children.size() + 1) * 4));
...
void _visitCallFunctionTemplate(NodeToken *nd, int regbase, bool isExtCall) {
  ...
  // printf(" %s %d %d\n",nd->_token->text.c_str(),
  // t->children.size(),t->getChildAtPos(1)->children.size()); for (int i = 0; i
  // < t->getChildAtPos(1)->children.size(); i++)
  ...
  for (int i = t->getChildAtPos(2)->children.size() - 1; i >= 0; i--) {
    ...
    for (int j = 0; j < i; j++) {
      if (nd->getChildAtPos(2)->getChildAtPos(j)->findMaxArgumentSize() - 1 >=
          i) {
        save_in_stack = true;
      }
    }
...

    {
      if (i < t->getChildAtPos(1)->children.size()) {
        if (t->getChildAtPos(1)->getChildAtPos(i)->getVarType()->_varType ==
            __Args__)
          convert = false;

        register_numl.duplicate();
        globalType.push(
            t->getChildAtPos(1)->getChildAtPos(i)->getVarType()->_varType);
        nd->getChildAtPos(2)->getChildAtPos(i)->visitNode();
        register_numl.pop();
      } else {
        register_numl.duplicate();
      ...
      if (nd->getChildAtPos(2)->getChildAtPos(i)->getVarType() != NULL and
          convert)
        translateType(
            globalType.get(),
            nd->getChildAtPos(2)->getChildAtPos(i)->getVarType()->_varType,
            register_numl.get());
      ...
      if (i < t->getChildAtPos(1)->children.size()) {
        ...
        if (_vartype == __Args__) {
          _vartype =
              t->getChildAtPos(2)->getChildAtPos(i)->getVarType()->_varType;
          isArg = true;
          /*
          bufferText->pop();
      ...
      if (_vartype == __float__) {
        ...
        if (t->getChildAtPos(2)
          ...
          for (int k = 0; k < t->getChildAtPos(2)
                                  ->getChildAtPos(i)
                                  ->getChildAtPos(0)
                                  ->getVarType()
                                  ->size;
...
void _visitcallFunctionNode(NodeToken *nd) {
  ...
  if (t->getChildAtPos(1)->children.size() >=
    ...
    if (t->getChildAtPos(1)->children.size() == 0 and
    ...
    for (int i = 0; i < t->getChildAtPos(1)->children.size(); i++) {
      ...
      if (t->getChildAtPos(1)->getChildAtPos(i)->isPointer) {
        ...
        register_numl.pop();
        int start =
            t->getChildAtPos(1)->getChildAtPos(i)->stack_pos - _STACK_SIZE;
        ...
        if (t->type == TokenUserDefinedVariableMemberFunction and i == 0) {
          ...
          if (!isStructFunction) {
            bufferText->addAfter(
                bufferText->sp.pop(),
                string_format("mov a11,a%d", register_numl.get()));
          } else {
            // bufferText->pop();
            bufferText->addAfter(bufferText->sp.pop(),
                                 string_format("mov a11,a3"));
          }
        } else {
          bufferText->addAfter(bufferText->sp.pop(),
        ...
        int start =
            t->getChildAtPos(1)->getChildAtPos(i)->stack_pos - _STACK_SIZE +
            t->getChildAtPos(1)->getChildAtPos(i)->getVarType()->total_size;
        int tot = t->getChildAtPos(1)->getChildAtPos(i)->getVarType()->size - 1;
        for (int j = 0;
             j < t->getChildAtPos(1)->getChildAtPos(i)->getVarType()->size;
             j++) {
          ...
          start -=
              t->getChildAtPos(1)->getChildAtPos(i)->getVarType()->sizes[tot -
                                                                         j];
          asmInstruction asmInstr =
              t->getChildAtPos(1)->getChildAtPos(i)->getVarType()->store[tot -
                                                                         j];
          ...
          if (!intest) {
            ...
            if (j ==
                t->getChildAtPos(1)->getChildAtPos(i)->getVarType()->size - 1) {
              // bufferText->sp.push(bufferText->get());
    ...
    bufferText->addAfter(string_format(
        "mov a10,a2")); // neded to find the external variables !!!!!!
    bufferText->addAfter(string_format("call8 @_%s", nd->getTokenText()));
  } else {
...
void _visitextGlobalVariableNode(NodeToken *nd) {
  ...
  if (!nd->isPointer) {
    bufferText->addAfter(
        string_format("movExt a%d,%s", point_regnum, nd->getTokenText()));
  } else {
    if (nd->children.size() > 0) {
      ...
      if (nd->children.size() > 1) {
        ...
        register_numl.get(), nd->children.size()));
        bufferText->addAfter(string_format("mov a%d,a10", register_numl.get()));
    */
        vector<string> tile;
        int nb = 0;
      ...
      bufferText->addAfter(string_format("movExt a%d,%s", register_numl.get(),
                                         nd->getTokenText()));
      bufferText->sp.push(bufferText->get());
      bufferText->sp.push(bufferText->get());
      register_numl.pop();
  ...
  if (nd->children.size() >= _TRIGGER) { // point_regnum;
    ...
    for (int i = 0; i < nd->children.size(); i++) {
      ...
      if (nd->getChildAtPos(i)->isPointer) {
        ...
        bufferText->addAfter(
            string_format("%s a%d,a1,%d", asmInstructionsName[asmInstr].c_str(),
                          reg_num, start));
      }
      reg_num++;
      /*
  ...
  if (nd->isPointer) {
    ...
    if (nd->children.size() > 0) {
      ...
      if (nd->type == TokenUserDefinedVariableMember or
        ...
        if (isStructFunction) {
          bufferText->addAfter(string_format("addi a%d,a%d,%d", point_regnum, 3,
