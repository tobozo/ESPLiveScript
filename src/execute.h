...
public:
  ...
  void execute(string prog) {
    ...
    error_message_struct res =
        executeBinary("@__footer", _executecmd, 9999, args);
...
static void _run_task(void *pvParameters) {
  ...
  if (exec->df.args.size() > 0) {
    error_message_struct res = executeBinary(
        "@__footer", exec->df.exe, exec->__run_handle_index, exec->args);

    res = executeBinary(exec->df.args[0], exec->df.exe,
                        exec->__run_handle_index, exec->args);
    if (res.error) {
      pushToConsole(res.error_message, true);
    }
  } else {
    error_message_struct res = executeBinary(
        "@__footer", exec->df.exe, exec->__run_handle_index, exec->args);
    res = executeBinary("@_main", exec->df.exe, exec->__run_handle_index,
                        exec->args);
    if (res.error) {
      pushToConsole(res.error_message, true);
    }
