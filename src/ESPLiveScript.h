...
void prettyPrint2(NodeToken nd, string ident) {
  /*
      printf("%s%s\t isPointer:%d\t asPointer:%d\t", ident.c_str(),
     nodeTypeNames[nd._nodetype].c_str(), nd.isPointer, nd.asPointer); //,
     ...
     varTypeEnumNames[nd.getVarType()->_varType].c_str(), nd._total_size,
     nd.stack_pos);
      }
      else
      {
...
public:
  ...
  Executable compile() {
    ...
    executable _executecmd =
        createExectutable(&footer, &header, &content, __parser_debug);
          ...
          if (Match(TokenCloseBracket)) {
            ...
            if (nb < _s) {
              ...
              Error.error_message = string_format(
                  "too many arguments expected less than %d got %d at %s", nb,
                  _s, linepos().c_str());
              next();
              return;
            }
            ...
            Error.error = 1;
            Error.error_message =
                string_format("expecting ]  or , %s at %s",
                              current()->getText(), linepos().c_str());
            next();
            return;
          }
      ...
      if (Match(TokenMember) && Match(TokenIdentifier, 1) &&
        ...
        if (!_asPointer)
          ...
          current_node->stack_pos =
              current_node->stack_pos + 1000 * v->starts[i];
        ...
        */
        ...
        current_node->getChildAtPos(current_node->children.size() - 1)
            ->getChildAtPos(2)
            ->getChildAtPos(0)
  ...
  void parseFunctionCall() {
    ...
    if (search_result == NULL) {
      if (struct_name.size() > 0) {
        sav_t.back().addText(string_format("%s.%s", struct_name.c_str(),
                                           sav_t.back().getText()));
        main_context.findFunction(&sav_t.back());
        isStructFunction = true;
        if (search_result == NULL) {
          Error.error = 1;
          Error.error_message =
              string_format("lkjlfunction %s not found %s",
                            sav_t.back().getText(), linepos().c_str());
          return;
        }
      } else {
        Error.error = 1;
    ...
    for (int i = 0; i < current_node->getChildAtPos(1)->children.size(); i++) {
      if (current_node->getChildAtPos(1)->getChildAtPos(i)->_vartype ==
          __Args__) {
  ...
  void parseStatement() {
    ...
    if (Match(TokenIdentifier) &&
      ...
      // current_cntx->_global->children.size(),
      ...
      if (Match(TokenEqual)) {
        ...
        if (nodeTokenList.get().type == TokenUserDefinedVariable) {
          ...
          next();
          //       current_node->getChildAtPos(current_node->children.size() -
  ...
  void parseDefFunction() {
    ...
    bool is_asm = false;
    // printf("entering function %s with
  ...
  void parseFactor() {
    ...
    Error.error = 1;
    Error.error_message =
        string_format(" impossible to find Token %s %s", current()->getText(),
                      linepos().c_str());
    next();
    return;
  }
