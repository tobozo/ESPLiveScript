...
error_message_struct parseASM(Text *_footer, Text *_header, Text *_content,
  ...
  string _d = string_format("Creation of an %d bytes binary and %d bytes data",
                            _instr_size, _size);
    ...
    if ((*it)->op != opCodeType::data && (*it)->op != opCodeType::number) {
      ...
      if ((*it)->align == true) {
        ...
        if ((add_instr & 3) != 0) {
          ...
          // it->name.c_str()); printf("one insert \n");
...
void dumpmem(uint32_t *dump) {
  ...
  for (int j = 0; j < 30; j++)
  {
    ...
    for (int i = 0; i < 2; i++)
    {
      printf(" %02x | %02x | %02x | %02x |", ((*dump)) & 0xFF, ((*dump) >> 8) &
  0xFF, ((*dump) >> 16) & 0xFF, ((*dump) >> 24) & 0xFF); dump++;
    }
  }
  printf("\n\n\n");
  */
...
void flagLabel32aligned(parsedLines *asm_parsed) {
  ...
  for (vector<result_parse_line *>::iterator it = asm_parsed->begin();
       it != asm_parsed->end(); it++) {
    if ((*it)->op == opCodeType::jump_32aligned ||
        (*it)->op == opCodeType::function_declaration) {
...
void createAbsoluteJump(uint8_t *exec, parsedLines *asm_parsed,
                        uint32_t address) {
  int nb_data = 0;
  for (vector<result_parse_line *>::iterator it = asm_parsed->begin();
       it != asm_parsed->end(); it++) {
    if ((*it)->op == opCodeType::data_label ||
      ...
      //  it--; // à cause de l'"it++"
      // printf("on veut mapper address:%s
      // %ld\r\n",(*it)->getText(),(*it)->bincode);
      // (*it)->address = content;
      // printf("absoluit line:%d\r\n",nb_data*4);
      uint32_t *new_adr =
          (uint32_t *)exec + nb_data; // (*asm_parsed)[i].address / 4;
...
executable createBinary(parsedLines *asm_parsed) {
  ...
  for (vector<result_parse_line *>::iterator it = asm_parsed->begin();
    ...
    if ((*it)->op == opCodeType::function_declaration) {
      ...
      if (index == -1) {
        exe.error.error = 1;
        exe.error.error_message = string_format(
            "Impossible to find start function :%s", (*it)->getText());
        return exe;
      } else {
        globalcall gc;
        ...
        gc.address =
            getInstrAtPos(index)->address; //(*asm_parsed)[index].address;
...
executable createExectutable(Text *_footer, Text *_header, Text *_content,
                             bool display) {
...
error_message_struct executeBinary(string function, executable ex,
                                   uint32_t handle, Arguments arguments) {
  ...
  res.error = 1;
  res.error_message =
      string_format("Impossible to execute %s: not found\n", function.c_str());
  return res;
}
